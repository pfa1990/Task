...
def drawPlanets():
    for planet in celestialBodies:
        ...
        surface.blit(solarsystem.images[planet["name"]],
...
def drawCurrentBody():
    ...
    surface.blit(solarsystem.images[currentBody["name"]],
        ...
        for otherPlanet in celestialBodies:
            if otherPlanet is not planet:
                direction = (otherPlanet["position"][0] - planet["position"][0],
                             ...
                             # The difference in the X, Y coordinates of the
                             # objects
                magnitude = math.hypot(otherPlanet["position"][0] -
                                       planet["position"][0],
                                       otherPlanet["position"][1] -
                                       planet["position"][1])
                            # The distance between the two objects
                nDirection = (direction[0] / magnitude, direction[1] /
                              magnitude)
                             # Normalised vector pointing in the direction of
                             # force
                ...
                strength = ((gravity * planet["mass"] * otherPlanet["mass"]) /
                           ...
                           # How strong is should the attraction be?

                appliedForce = (nDirection[0] * strength, direction[1] *
                                strength)
                ...
                if drawAttractions is True:
                    pygame.draw.line(surface, (255, 255, 255),
                                     (planet["position"][0],
                                     planet["position"][1]),
                                     (otherPlanet["position"][0],
                                     otherPlanet["position"][1]), 1)
...
def handleMouseDown():
    ...
    if (mousePosition[1] >= 687):
        ...
        if newPlanet is not False:
            currentBody = solarsystemMakeNewPlanet(newPlanet)
...
while True:
    ...
    # Draw the UI, update the movement of the planets, draw the planets in their
    # new positions.
    drawUI()
    calculateMovement()
    drawPlanets()
    ...
    if currentBody is not None:
        ...
        if mouseDown is False:
            currentBody["velocity"][0] = (mousePosition[0] -
                                          previousMousePosition[0]) / 4
            currentBody["velocity"][1] = (mousePosition[1] -
        ...
        # Store the previous mouse coordinates to create a vector when we
        # release a new planet
