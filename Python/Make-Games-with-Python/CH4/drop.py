...
def drawPlayer():
    pygame.draw.rect(surface, (255, 0, 0), (player["x"], player["y"],
...
def movePlayer():
    ...
    if surface.get_at((player["x"], player["y"] + player["height"])) == (0, 0,
        0, 255):
        leftOfPlayerOnPlatform = False

    if (surface.get_at((player["x"] + player["width"], player["y"] +
        player["height"])) == (0, 0, 0, 255)):
        rightOfPlayerOnPlatform = False

    if (leftOfPlayerOnPlatform is False and rightOfPlayerOnPlatform is False and
        ...
        while foundPlatformTop is False:
            if (surface.get_at((player["x"], (player["y"] + player["height"]) -
                yOffset)) == (0, 0, 0, 255)):
                player["y"] -= yOffset
    ...
    if rightDown is True:
        if (player["x"] + player["width"] < windowWidth and (player["x"] +
            player["width"]) + 5 < windowWidth):
            player["x"] += 5
        elif (player["x"] + player["width"] < windowWidth and
            (player["x"] + player["width"]) + 5 > windowWidth):
...
def drawPlatforms():
    for platform in gamePlatforms:
        pygame.draw.rect(surface, (255, 255, 255), (platform["pos"][0],
                         platform["pos"][1], windowWidth, 10))
        pygame.draw.rect(surface, (0, 0, 0), (platform["gap"],
                         platform["pos"][1], 40, 10))
