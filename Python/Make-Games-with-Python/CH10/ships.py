...
class Player():
    ...
    def fire(self):
        self.bullets.append(projectiles.Bullet(self.x + self.width / 2, self.y,
                            self.pygame, self.surface, self.bulletSpeed,
                            self.bulletImage))
    ...
    def checkForHit(self, thingToCheckAgainst):
        ...
        for idx, b in enumerate(self.bullets):
            if (b.x > thingToCheckAgainst.x and b.x < thingToCheckAgainst.x +
               thingToCheckAgainst.width):
                if (b.y > thingToCheckAgainst.y and b.y < thingToCheckAgainst.y
                   + thingToCheckAgainst.height):
