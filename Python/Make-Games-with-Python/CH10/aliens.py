...
def launchWave():
    ...
    thisLevel = levels.level[currentLevel]["structure"]
    ...
    if currentWave < len(thisLevel):
        ...
        for idx, enemyAtThisPosition in enumerate(thisWave):
            if enemyAtThisPosition is 1:
                enemyShips.append(ships.Enemy(((windowWidth / len(thisWave)) *
                                              idx), -60, pygame, surface, 1))

    elif currentLevel + 1 < len(levels.level) :
        currentLevel += 1
        currentWave = 0
...
def drawGame():
    ...
    pygame.draw.rect(surface, healthColor[whichColor], (0, windowHeight - 5,
                     (windowWidth / ship.maxHealth) * ship.health, 5))
    pygame.draw.rect(surface, (62, 145, 180), (0, windowHeight - 10,
                     (windowWidth / ship.maxShields) * ship.shields, 5))
...
while True:
    ...
    if gameStarted is True and gameOver is False:
        ...
        if mouseStates[0] is 1:
            if (mousePosition[0] > 445 and mousePosition[0] < 580 and
    ...
    for event in GAME_EVENTS.get():
        if event.type == pygame.KEYDOWN:
            ...
            if event.key == pygame.K_SPACE:
                if (gameStarted is True and gameOver is True or gameStarted is
                   True and gameWon is True):
                    restartGame()
    if (timeTick - lastSpawn > levels.level[currentLevel]["interval"] * 1000
       and gameStarted is True and gameOver is False):
        launchWave()
